    ...
    while true do
        ...
        if isPlayerNearTent then
        	...
        	if not hasAlertInteractKey and Tent.isCreated then
				...
				TriggerEvent("redemrp_notification:start", "Press G to interact with the Camp" , 4, "success")
			end

            if IsControlPressed(0, 0x760A9C6F) then -- Open Menu if Player hit G near the Tent, but it wont open if SHIFT+G is pressed (bc_sitchair)
                    	...
                    	if not Tent.Blip then
                            	...
                            	if not Tent.isHitchingPostCreated then
	                        		if WarMenu.Button('Craft Hitch Post') then
	                        			...
	                        			local propInFrontOfPed = IsThereAnyPropInFrontOfPed(playerPed,1.5,2.0)
	                                    	...
	                                    	exports['progressBars']:startUI(7000, "Setting Hitch post...")
		                                    for i=1,5 do
		                                        playAnim("mini_games@story@beechers@build_floor@john","hammer_loop_good")
		                                        Citizen.Wait(1500)
		                                    end
                            ...
                            if WarMenu.Button('Craft Tent') then
                                ...
                                if equals(Tent.ActualItems,Tent.RequiredItems,false) then 
                                    ...
                                    Citizen.Wait(2500)
                                    exports['progressBars']:startUI(7000, "Finalizing Tent...")
                                    for i=1,5 do
                                        playAnim("mini_games@story@beechers@build_floor@john","hammer_loop_good")
                                        Citizen.Wait(1500)
                                    end
                            ...
                            if WarMenu.Button('Build Campfire') then
                               -- Same as before, it fill the actual items so it will let you craft the campfire
                                ...
                                if equals(Campfire.ActualItems,Campfire.RequiredItems,false) then -- If two tables are equals
                                    ...
                                    exports['progressBars']:startUI(7000, "Building Campfire...")
                                    for i=1,5 do
                                        playAnim("mini_games@story@beechers@build_floor@john","hammer_loop_good")
                                        Citizen.Wait(1500)
                                    end
	                        	 ...
	                        	 if WarMenu.Button('Set Up Seat') then 
	                        	 	   ...
	                        	 	   local propInFrontOfPed = IsThereAnyPropInFrontOfPed(playerPed,1.5,2.0)
                                        	...
                                        	TriggerEvent("redemrp_notification:start", "You can't build here" , 2, "warning")
                        	 	   			...
                        	 	   			exports['progressBars']:startUI(7000, "Setting up Seat...")
		                                    for i=1,5 do
		                                        playAnim("mini_games@story@beechers@build_floor@john","hammer_loop_good")
		                                        Citizen.Wait(1500)
		                                    end
...
RegisterCommand('camp', function(source, args, rawCommand)
    ...
    if Tent.Prop~=nil then --  If tent already exists, it deletes it
     ...
     exports['progressBars']:startUI(15700, "Setting Structure...")
